...
if __name__ == '__main__':
    ...
    # 查看视频信息
    print("➡️ 查看视频信息")
    os.system("ffprobe -hide_banner -i out_mp4/out_muxered.mp4")

    if len(args) > 0 and args[0] == "play-video":
        print("➡️ 播放视频")
        os.system("ffplay -loop 0  out_mp4/out_muxered.mp4")
    #运行上传视频脚本
    ...
    if os.name == "nt":
        # windows
        os.system(
            'powershell pwd && cd ../VideoPlayground && powershell Remove-Item "out.mp3", "out_zoompan.mp4"')
        os.system(
            'powershell pwd && cd ../VideoPlayground && powershell  Remove-Item "img","file_list.txt","img_file_list_file.txt" -Recurse')
        os.system("powershell pwd && cd ../VideoPlayground && powershell  Remove-Item v_out_mp3 -Recurse")
    else:
